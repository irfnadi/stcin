  ...
  try {
    ...
    try {
      if (global.DATABASE._data.users[m.sender]) {
        ...
        ) global.DATABASE._data.users[m.sender].limit = 200
      ...
      let _prefix = plugin.customPrefix ? plugin.customPrefix : conn.prefix ? conn.prefix : global.prefix
        ...
        if (!isPrems && global.DATABASE._data.users[m.sender].limit < 1 && plugin.limit) {
          this.reply(m.chat, `Limit anda habis, silahkan Chat Owner Bot *${usedPrefix}owner*`, m)
          continue
        }
        try {
          await plugin(m, {
...
global.mods = ['6282286425538@s.whatsapp.net']
global.prems = []
